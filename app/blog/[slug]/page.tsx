...
export default async function ArticlePage({ params }: PageProps) {
    ...
    // Chart article — specialised layout
    ...
    return (
        <main className="mx-auto max-w-3xl px-4 pt-20 sm:px-6">
            ...
            <header className="pb-6">
                <span
                    className="mb-3 inline-block px-2 py-0.5 text-xs font-semibold"
                    style={{ background: badgeStyle.bg, color: badgeStyle.text, borderRadius: 0 }}
                >
                    {article.category}
                </span>
            ...
            {article.imageUrl && (
                <div
                    className="mb-8 h-64 w-full overflow-hidden sm:h-80"
                    style={{
                        background: `url(${article.imageUrl}) center/cover no-repeat`,
                    }}
                />
            )}
